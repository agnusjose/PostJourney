...
HOLD_TIME = 1.5
TARGET_CYCLES = 5
YAW_THRESHOLD = 0.005     # very low — any noticeable yaw counts
PITCH_THRESHOLD = 0.010
NEUTRAL_EPS = 0.020       # tight enough to not overlap with yaw threshold
SMOOTH_WINDOW = 3
...
def detect_direction(yaw, pitch, baseline):
    ...
    # Both axes within neutral tolerance → head is roughly centered
    if abs_dy < NEUTRAL_EPS and abs_dp < NEUTRAL_EPS:
        return "neutral"

    # DOMINANT AXIS wins — whichever has the bigger change determines direction
    if abs_dy >= abs_dp:
        # Yaw is dominant → left/right
        direction = "right" if dy > 0 else "left"
        print(f"  → YAW dominant: dy={dy:.4f} dp={dp:.4f} → {direction}")
        return direction
    else:
        # Pitch is dominant → up/down
        direction = "down" if dp > 0 else "up"
        print(f"  → PITCH dominant: dy={dy:.4f} dp={dp:.4f} → {direction}")
        return direction
