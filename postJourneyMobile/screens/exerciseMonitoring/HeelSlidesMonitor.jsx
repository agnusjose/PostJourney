...
export default function HeelSlidesMonitor() {
  const navigation = useNavigation();

  const [instruction, setInstruction] = useState(
    "Lie down and position yourself in front of the camera"
  );
  ...
  const isHighPriority = (text) => {
    const priorities = [
      "slide", "hold", "complete", "ready", "getting ready",
      "heel", "surface", "hips", "starting position",
    ];
  ...
  useEffect(() => {
    ...
    startWebRTCStream(POSE_API_BASE_URL, "heel_slides", (s) => setLocalStream(s))
      .then((stopFunc) => {
        stopStream = stopFunc;
        setStreamReady(true);
      })
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>HEEL SLIDES AI</Text>
