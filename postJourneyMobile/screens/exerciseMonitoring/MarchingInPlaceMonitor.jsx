...
export default function MarchingInPlaceMonitor() {
  ...
  const isHighPriority = (text) => {
    const priorities = [
      "lift", "march", "switch", "complete", "ready", "getting ready",
      "stand tall", "opposite arm", "higher", "keep marching",
    ];
  ...
  useEffect(() => {
    ...
    if (cleanText && cleanText !== lastSpokenRef.current) {
      Speech.stop();
      Speech.speak(cleanText, { language: "en-US", rate: 1.0, pitch: 1.0 });
  ...
  useEffect(() => {
    ...
    startWebRTCStream(POSE_API_BASE_URL, "marching_in_place", (s) => setLocalStream(s))
      .then((stopFunc) => {
        stopStream = stopFunc;
        setStreamReady(true);
      })
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>MARCHING IN PLACE AI</Text>
