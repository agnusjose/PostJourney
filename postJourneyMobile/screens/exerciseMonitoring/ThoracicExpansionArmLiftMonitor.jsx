...
export default function ThoracicExpansionArmLiftMonitor() {
  ...
  const isHighPriority = (text) => {
    const priorities = [
      "inhale", "exhale", "hold", "raise", "lower", "complete",
      "cycle", "stretch", "breathe", "done", "arms",
    ];
  ...
  useEffect(() => {
    ...
    startWebRTCStream(POSE_API_BASE_URL, "thoracic_expansion_arm_lift", (s) => setLocalStream(s))
      .then((stopFunc) => {
        stopStream = stopFunc;
        setStreamReady(true);
      })
        ...
        if (data.completed && !completedHandledRef.current) {
          ...
          setTimeout(() => {
            navigation.replace("ExerciseCompleted", {
              exerciseName: "Thoracic Expansion Arm Lift",
              reps: data.reps || reps,
              targetReps: 10,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>THORACIC EXPANSION AI</Text>
          <Text style={styles.feedbackText}>{instruction}</Text>

          <View style={styles.repRow}>
            <View style={styles.repBadge}>
              <Text style={styles.repCount}>{reps}</Text>
              <Text style={styles.repLabel}>/{TARGET_REPS} CYCLES</Text>
            </View>
          </View>
