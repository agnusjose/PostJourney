...
export default function SeatedShoulderFlexionMonitor() {
  ...
  const isHighPriority = (text) => {
    const priorities = ["raise", "hold", "lower", "complete", "rep", "done", "stretch", "arms"];
  ...
  useEffect(() => {
    ...
    let stopStream;
    startWebRTCStream(POSE_API_BASE_URL, "seated_shoulder_flexion", (s) => setLocalStream(s))
      ...
      try {
        ...
        if (data.completed && !completedHandledRef.current) {
          ...
          setTimeout(() => navigation.replace("ExerciseCompleted", {
            exerciseName: "Seated Shoulder Flexion",
            reps: data.reps || reps,
            targetReps: 10,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>SEATED SHOULDER FLEXION AI</Text>
