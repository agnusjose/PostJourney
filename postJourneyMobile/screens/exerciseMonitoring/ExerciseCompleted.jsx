...
export default function ExerciseCompleted() {
  ...
  const performanceLevel = route.params?.performanceLevel ?? 0;

  // Auto-save exercise session once on mount
  useEffect(() => {
    if (savedRef.current) return;
    savedRef.current = true;

    const patientId = user?.userId || user?._id || "unknown";
    const today = new Date().toISOString().split("T")[0];

    saveExercise({
      patientId,
      exerciseName,
      repetitionsCompleted: typeof reps === "number" ? reps : 0,
