...
export default function StaticStandingMonitor() {
  ...
  const isHighPriority = (text) => {
    const priorities = ["stand", "hold", "phase", "complete", "done", "tandem", "single", "feet", "lift"];
  ...
  useEffect(() => {
    ...
    // Strip countdown so "Hold (25s)" and "Hold (24s)" are the same
    const cleanText = instruction.replace(/\(\d+s remaining\)/g, "").replace(/\d+s remaining/g, "").replace(/in \d+s/g, "").trim();
  ...
  useEffect(() => {
    ...
    let stopStream;
    startWebRTCStream(POSE_API_BASE_URL, "static_standing", (s) => setLocalStream(s))
      ...
      try {
        ...
        if (data.completed && !completedHandledRef.current) {
          ...
          setTimeout(() => navigation.replace("ExerciseCompleted", {
            exerciseName: "Static Standing Endurance",
            reps: data.reps || reps,
            targetReps: TOTAL_PHASES,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>STATIC STANDING ENDURANCE AI</Text>
          <Text style={styles.feedbackText}>{instruction}</Text>
          <View style={styles.repRow}>
            <View style={styles.repBadge}>
              <Text style={styles.repCount}>{reps}</Text>
              <Text style={styles.repLabel}>/{TOTAL_PHASES} PHASES</Text>
            </View>
          </View>
