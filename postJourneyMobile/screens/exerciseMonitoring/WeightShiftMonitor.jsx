...
export default function WeightShiftMonitor() {
  ...
  const isHighPriority = (text) => {
    const priorities = ["shift", "hold", "center", "complete", "rep", "done", "left", "right"];
  ...
  useEffect(() => {
    ...
    const cleanText = instruction.replace(/\(\d+s\)/g, "").replace(/\(Rep \d+\/\d+\)/g, "").trim();
  ...
  useEffect(() => {
    ...
    let stopStream;
    startWebRTCStream(POSE_API_BASE_URL, "weight_shift", (s) => setLocalStream(s))
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (typeof data.progress === "number") {
          ...
          setMaxProgress((prev) => Math.max(prev, data.progress));
        }
          ...
          setTimeout(
            () =>
              navigation.replace("ExerciseCompleted", {
                exerciseName: "Weight Shift",
                reps: data.reps || reps,
                targetReps: 12,
                performanceLevel: Math.round((maxProgress || 0) * 100),
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>WEIGHT SHIFT AI</Text>
