...
export default function StraightLegRaiseMonitor() {
  ...
  const isHighPriority = (text) => {
    const priorities = ["raise", "lift", "hold", "lower", "complete", "rep", "done", "straight"];
  ...
  useEffect(() => {
    ...
    const cleanText = instruction.replace(/\(\d+s\)/g, "").replace(/\d+s remaining/g, "").replace(/in \d+s/g, "").trim();
  ...
  useEffect(() => {
    ...
    let stopStream;
    startWebRTCStream(POSE_API_BASE_URL, "straight_leg_raise", (s) => setLocalStream(s))
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>STRAIGHT LEG RAISE AI</Text>
