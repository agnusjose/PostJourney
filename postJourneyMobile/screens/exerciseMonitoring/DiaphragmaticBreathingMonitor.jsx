...
export default function DiaphragmaticBreathingMonitor() {
  ...
  const isHighPriority = (text) => {
    const priorities = ["inhale", "exhale", "breathe", "complete", "hold", "cycle", "belly"];
  ...
  useEffect(() => {
    ...
    // Strip countdown and cycle numbers so "Inhale (4s)" and "Inhale (3s)" are the same
    const cleanText = instruction.replace(/\(\d+s\)/g, "").replace(/Cycle \d+\/\d+/g, "").replace(/starting in \d+s/g, "starting").trim();
    if (!cleanText || cleanText.length < 5 || cleanText === lastSpokenRef.current) return;

    const highPriority = isHighPriority(instruction);
    const now = Date.now();
    const elapsed = now - lastSpeakTimeRef.current;
    if (!highPriority && elapsed < 4000) return;

    lastSpokenRef.current = cleanText;
    lastSpeakTimeRef.current = now;
    Speech.stop();
    Speech.speak(cleanText, { language: "en-US", rate: 0.85, pitch: 1.0 });
  }, [instruction]);
  ...
  useEffect(() => {
    ...
    let stopStream;
    startWebRTCStream(POSE_API_BASE_URL, "diaphragmatic_breathing", (s) => setLocalStream(s))
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>DIAPHRAGMATIC BREATHING AI</Text>
          <Text style={styles.feedbackText}>{instruction}</Text>
          <View style={styles.repRow}>
            <View style={styles.repBadge}>
              <Text style={styles.repCount}>{reps}</Text>
              <Text style={styles.repLabel}>/{TARGET_CYCLES} CYCLES</Text>
            </View>
          </View>
