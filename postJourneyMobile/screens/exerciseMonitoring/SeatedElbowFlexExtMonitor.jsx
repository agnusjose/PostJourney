...
export default function SeatedElbowFlexExtMonitor() {
  ...
  const isHighPriority = (text) => {
    const priorities = ["straighten", "bend", "extend", "complete", "rep", "done", "good"];
  ...
  useEffect(() => {
    ...
    let stopStream;
    startWebRTCStream(POSE_API_BASE_URL, "seated_elbow_flex_ext", (s) => setLocalStream(s))
      ...
      try {
        ...
        if (data.completed && !completedHandledRef.current) {
          ...
          setTimeout(() => navigation.replace("ExerciseCompleted", {
            exerciseName: "Seated Elbow Flexion-Extension",
            reps: data.reps || reps,
            targetReps: 15,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>ELBOW FLEXION-EXTENSION AI</Text>
