...
export default function SeatedTrunkFlexExtMonitor() {
  ...
  const isHighPriority = (text) => {
    const p = [
      "bend", "forward", "core", "engage", "slow", "upright",
      "complete", "great", "rep", "ready", "return",
    ];
  ...
  useEffect(() => {
    ...
    let stopStream;
    startWebRTCStream(POSE_API_BASE_URL, "seated_trunk_flexion_extension", (s) =>
      setLocalStream(s)
    )
      .then((f) => {
        stopStream = f;
      ...
      try {
        ...
        if (data.completed && !completedHandledRef.current) {
          ...
          setTimeout(() => navigation.replace("ExerciseCompleted", {
            exerciseName: "Seated Trunk Flexion–Extension",
            reps: data.reps || reps,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>TRUNK FLEX / EXT AI</Text>
