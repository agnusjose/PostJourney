...
export default function MiniSquatMonitoring() {
  ...
  const isHighPriority = (text) => {
    const p = [
      "rep", "complete", "great", "good", "bend",
      "chest", "deep", "stand", "rise",
    ];
  ...
  useEffect(() => {
    ...
    if (instruction.length > 3) {
      Speech.stop();
      Speech.speak(instruction, { language: "en-US", rate: 0.95, pitch: 1.0 });
    }
  }, [instruction]);
  ...
  useEffect(() => {
    ...
    let stopStream;
    startWebRTCStream(POSE_API_BASE_URL, "mini_squat", (s) =>
      setLocalStream(s)
    )
      .then((f) => {
        stopStream = f;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.hudContainer}>
        <View style={styles.feedbackCard}>
          <Text style={styles.title}>MINI SQUAT AI</Text>
