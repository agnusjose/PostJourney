...
const BASE_URL_IMAGE = 'http://192.168.91.72:5000';
...
export default function ConsultDoctor({ navigation, embedded }) {
    ...
    const handleBookPress = async (doctor) => {
        ...
        fetchSlots(doctor._id, today);
    };

    const fetchSlots = async (doctorId, dateStr) => {
        setSlotsLoading(true);
        try {
            const response = await axios.get(`${BASE_URL}/doctors/${doctorId}/slots?date=${dateStr}`);

            // Filter out past time slots only if the selected date is today
            const now = new Date();
            const todayStr = now.toISOString().split('T')[0];
            const isToday = dateStr === todayStr;

            const filteredSlots = response.data.map((slot) => {
                if (!isToday) return slot; // Future dates: all unbooked slots are available
    ...
    const handleDateSelect = (dateStr) => {
        ...
        setSelectedSlot(null);
        if (selectedDoctor) {
            fetchSlots(selectedDoctor._id, dateStr);
        }
    };
    ...
    const confirmBooking = async () => {
        ...
        try {
            ...
            navigation.navigate("PaymentScreen", {
                type: 'consultation',
                amount: 200, // Fixed consultation fee (patient pays 200)
                doctorId: selectedDoctor._id,
                bookingData: payload
            });
        } catch (error) {
        ...
        <View style={[styles.card, !item.isOnline && styles.cardOffline]}>
            <View style={styles.cardHeader}>
                ...
                <View style={styles.cardContent}>
                    ...
                    <Text style={styles.fee}>Fee: ₹200</Text>
                    {item.licenseNumber ? <Text style={styles.experience}>🪪 License: {item.licenseNumber}</Text> : null}
                </View>
            </View>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 15 }}>
                            {getNext7Days().map((day) => (
                                <TouchableOpacity
                                    ...
                                    style={[
                                        styles.dateChip,
                                        selectedDate === day.dateStr && styles.dateChipSelected
                                    ]}
                                    onPress={() => handleDateSelect(day.dateStr)}
                                >
                                    <Text style={[
                                        styles.dateChipDay,
                                        selectedDate === day.dateStr && styles.dateChipTextSelected
                                    ]}>{day.dayName}</Text>
                                    <Text style={[
                                        styles.dateChipNum,
                                        selectedDate === day.dateStr && styles.dateChipTextSelected
...
const styles = StyleSheet.create({
    ...
    dateChipNum: { fontSize: 20, color: '#0A3D52', fontWeight: '800', marginVertical: 2 },
    dateChipMonth: { fontSize: 11, color: '#64748b', fontWeight: '500' },
