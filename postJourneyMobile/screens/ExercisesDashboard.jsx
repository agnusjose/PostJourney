...
import { getPatientPrescriptions } from "../services/exerciseService";
import { EXERCISES as EXERCISE_MAP } from "../data/exercises";
...
export default function ExercisesDashboard({ navigation }) {
  ...
  const fetchRecommended = useCallback(async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setRecommended(flattened);
      }
    } catch (err) {
      console.error("Failed to load recommended exercises:", err);
    } finally {
  ...
  const startExercise = (exerciseKey) => {
    const def = EXERCISE_MAP[exerciseKey];
    if (def?.monitorScreen) {
      navigation.navigate(def.monitorScreen, { userId });
    }
  };
              ...
              {recommended.map((ex) => (
                ...
                >
                  ...
                  <View style={styles.cardText}>
                    ...
                    <Text style={styles.cardSub}>Prescribed by {ex.doctorName}</Text>
                  </View>
                  <MaterialCommunityIcons
                    name="arrow-right"
                    size={20}
