...
export default function DoctorProfileCompletion({ route, navigation }) {
    ...
    const handleSubmit = async () => {
        ...
        if (licenseDoc) {
            const licFilename = licenseDoc.split('/').pop();
            const licMatch = /\.(\w+)$/.exec(licFilename);
            const licType = licMatch ? `image/${licMatch[1]}` : `image`;
            formData.append("licenseDocument", { uri: licenseDoc, name: licFilename, type: licType });
        }
    ...
    return (
        <ScrollView contentContainerStyle={styles.container}>
            ...
            <TouchableOpacity
                style={[styles.imagePicker, { width: '100%', height: 60, borderRadius: 8 }]}
                onPress={async () => {
                    let result = await ImagePicker.launchImageLibraryAsync({
                        mediaTypes: ['images'],
                        allowsEditing: false,
                    ...
                    if (!result.canceled) {
                        setLicenseDoc(result.assets[0].uri);
                    }
                }}
            >
                <Text style={styles.imagePlaceholder}>
                    {licenseDoc ? '✅ License document selected' : 'Upload License Document'}
                </Text>
            </TouchableOpacity>
