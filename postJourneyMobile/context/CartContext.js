...
export function CartProvider({ children }) {
  ...
  const loadUserCart = async () => {
    try {
      ...
      console.log("🔑 Looking for cart with key:", storageKey);
      ...
      if (storedCart) {
        ...
        console.log("📦 Cart loaded with", cartWithSelection.length, "items");
      } else {
        ...
        if (oldCartData) {
          console.log("🔄 Found old global cart, migrating...");
          ...
          setCart(cartWithSelection);
          console.log("✅ Migrated old cart with", cartWithSelection.length, "items");
        } else {
  ...
  useEffect(() => {
    console.log("👤 AuthContext user changed to:", user?.userId);
  ...
  useEffect(() => {
    if (!initialized) {
      console.log("⏸️ Skipping save - cart not initialized yet");
      return;
    }

    if (!user?.userId) {
      console.log("⏸️ Skipping save - no user logged in");
      return;
    }
  ...
  const prepareForImmediateBooking = async (equipmentId, quantity = 1) => {
    setLoading(true);
    try {
      console.log("🚀 Preparing immediate booking for:", equipmentId);
      ...
      console.log("✅ Booking item prepared:", bookingItem);
