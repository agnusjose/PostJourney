...
import exerciseTrackingRoutes from "./routes/exerciseTrackingRoutes.js";
...
import PatientHealth from "./models/PatientHealth.js";
import ExerciseSession from "./models/ExerciseSession.js";
import cron from "node-cron";
import WristbandOrder from "./models/WristbandOrder.js";
...
app.use("/api", exerciseTrackingRoutes);
...
const dirs = ["uploads/doctors", "uploads/messages", "uploads/documents", "uploads/licenses"];
dirs.forEach(dir => {
...
const licenseStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => cb(null, "lic-" + Date.now() + path.extname(file.originalname))
});
...
app.post("/api/doctor/complete-profile", doctorUpload.fields([
  ...
  try {
    ...
    if (req.files && req.files.doctorImage && req.files.doctorImage[0]) {
      user.doctorImage = `/uploads/doctors/${req.files.doctorImage[0].filename}`;
    }

    if (req.files && req.files.licenseDocument && req.files.licenseDocument[0]) {
      user.licenseDocumentUrl = `/uploads/licenses/${req.files.licenseDocument[0].filename}`;
    }
...
// Get health record for a patient
app.get("/api/patient/:id/health-record", async (req, res) => {
  try {
    let record = await PatientHealth.findOne({ patientId: req.params.id });
    if (!record) {
      record = { bpRecords: [], sugarRecords: [], cholesterolRecords: [], diseaseHistory: "", allergies: "", currentMedications: "", notes: "" };
    }
...
// Update general health info (disease history, allergies, medications)
app.put("/api/patient/:id/health-record", async (req, res) => {
  try {
    const { diseaseHistory, allergies, currentMedications, notes } = req.body;
    let record = await PatientHealth.findOne({ patientId: req.params.id });
    if (!record) {
      record = new PatientHealth({ patientId: req.params.id });
    }
    if (diseaseHistory !== undefined) record.diseaseHistory = diseaseHistory;
...
// Add a BP reading
app.post("/api/patient/:id/health-record/bp", async (req, res) => {
  try {
    const { systolic, diastolic, notes } = req.body;
    if (!systolic || !diastolic) return res.status(400).json({ success: false, error: "systolic and diastolic required" });
    let record = await PatientHealth.findOne({ patientId: req.params.id });
    if (!record) record = new PatientHealth({ patientId: req.params.id });
    record.bpRecords.push({ systolic, diastolic, notes, date: new Date() });
    await record.save();
    res.json({ success: true, record });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
...
// Add a sugar reading
app.post("/api/patient/:id/health-record/sugar", async (req, res) => {
  try {
    const { level, type, notes } = req.body;
    if (!level) return res.status(400).json({ success: false, error: "level required" });
    let record = await PatientHealth.findOne({ patientId: req.params.id });
    if (!record) record = new PatientHealth({ patientId: req.params.id });
    record.sugarRecords.push({ level, type: type || "random", notes, date: new Date() });
    await record.save();
    res.json({ success: true, record });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
...
// Add a cholesterol reading
app.post("/api/patient/:id/health-record/cholesterol", async (req, res) => {
  try {
    const { level, notes } = req.body;
    if (!level) return res.status(400).json({ success: false, error: "level required" });
    let record = await PatientHealth.findOne({ patientId: req.params.id });
    if (!record) record = new PatientHealth({ patientId: req.params.id });
    record.cholesterolRecords.push({ level, notes, date: new Date() });
    await record.save();
    res.json({ success: true, record });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
...
// Exercise reports - daily
app.get("/api/exercises/reports/daily/:patientId", async (req, res) => {
  try {
    ...
    res.json({ success: true, date: dateStr, sessions, summary: { totalSessions: sessions.length, totalDuration, totalReps } });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});

// Exercise reports - weekly
app.get("/api/exercises/reports/weekly/:patientId", async (req, res) => {
  try {
    const dateStr = req.query.date || new Date().toISOString().split('T')[0];
    const end = new Date(dateStr);
    end.setHours(23, 59, 59, 999);
    ...
    sessions.forEach(s => {
      ...
      if (!byDay[day]) byDay[day] = { sessions: 0, duration: 0, reps: 0 };
      byDay[day].sessions++;
      byDay[day].duration += s.duration || 0;
      byDay[day].reps += s.reps || 0;
    });
    res.json({ success: true, startDate: start.toISOString().split('T')[0], endDate: dateStr, byDay, summary: { totalSessions: sessions.length, totalDuration, totalReps } });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
// ========== CONSULTATION REMINDER CRON ==========
// Runs every minute, checks for consultations starting in ~10 minutes
cron.schedule("* * * * *", async () => {
  try {
    const now = new Date();
    ...
    // Find today's consultations that aren't cancelled/completed
    const consultations = await Consultation.find({
      consultationDate: new Date(todayStr),
      status: { $nin: ["cancelled", "completed"] },
    });

    for (const c of consultations) {
      if (!c.timeSlot) continue;

      // Parse slot time
      const parts = c.timeSlot.trim().split(" ");
      ...
      // Build IST slot time as minutes from midnight
      const slotMinutes = hours * 60 + minutes;
      const currentISTMinutes = istNow.getHours() * 60 + istNow.getMinutes();
      const diff = slotMinutes - currentISTMinutes;

      // If consultation is 9-10 minutes away, send reminder
      if (diff >= 9 && diff <= 10) {
        // Check if we already sent a reminder for this consultation
        ...
        const msg = `Your consultation is in 10 minutes at ${c.timeSlot}`;

        // Notify patient
        await Notification.create({
          userId: c.patientId,
          message: `${msg} with Dr. ${c.doctorName}`,
          type: "consultation_reminder",
          consultationId: c._id,
        });
        ...
        await Notification.create({
          userId: c.doctorId,
          message: `${msg} with patient ${c.patientName}`,
          type: "consultation_reminder",
          consultationId: c._id,
        });

        console.log(`🔔 Reminder sent for consultation ${c._id}`);
      }
    }
  } catch (err) {
...
// ========== WRISTBAND DEVICE ROUTES ==========

// Create wristband order
app.post("/api/wristband/order", async (req, res) => {
  try {
    const { patientId, patientName, email, quantity, deliveryAddress, contactPhone, notes } = req.body;

    if (!patientId || !patientName || !email || !deliveryAddress || !contactPhone) {
      return res.json({ success: false, message: "All required fields must be provided" });
    }

    const order = new WristbandOrder({
      patientId,
      patientName,
      email,
      quantity: quantity || 1,
      unitPrice: 500,
      totalAmount: 500 * (quantity || 1),
      deliveryAddress,
      contactPhone,
      notes: notes || "",
    });
    ...
    try {
      await Notification.create({
        userId: patientId,
        message: "Your HealthMonitor Pro wristband order has been placed successfully!",
        type: "wristband_order",
      });
    } catch (e) { console.log("Notification error:", e.message); }

    return res.json({
      success: true,
      message: "Wristband order placed successfully",
      orderId: order._id,
      order,
    });
  } catch (err) {
...
// Get patient's wristband orders
app.get("/api/wristband/orders/:patientId", async (req, res) => {
  try {
    const orders = await WristbandOrder.find({ patientId: req.params.patientId })
      .sort({ createdAt: -1 });
...
// Get single wristband order
app.get("/api/wristband/order/:orderId", async (req, res) => {
  try {
    const order = await WristbandOrder.findById(req.params.orderId);
...
// Check if patient has a delivered wristband + active orders
app.get("/api/wristband/has-device/:patientId", async (req, res) => {
  try {
    const deliveredOrder = await WristbandOrder.findOne({
      patientId: req.params.patientId,
      status: "delivered",
    });

    // Also check for active (non-delivered, non-cancelled) orders
    const activeOrder = await WristbandOrder.findOne({
      patientId: req.params.patientId,
      status: { $in: ["pending", "confirmed", "shipped", "out-for-delivery"] },
    }).sort({ createdAt: -1 });

    return res.json({
      success: true,
      hasDevice: !!deliveredOrder,
...
// Patient: Cancel wristband order
app.put("/api/wristband/order/:orderId/cancel", async (req, res) => {
  try {
    const order = await WristbandOrder.findById(req.params.orderId);
    if (!order) return res.json({ success: false, message: "Order not found" });
    ...
    if (!["pending", "confirmed"].includes(order.status)) {
      return res.json({
        success: false,
        message: `Cannot cancel order in "${order.status}" status. Contact support.`,
      });
    }
    ...
    try {
      await Notification.create({
        ...
        message: "Your wristband order has been cancelled successfully.",
        type: "wristband_cancelled",
      });
    } catch (e) { console.log("Notification error:", e.message); }
...
// Process wristband payment (dummy)
app.post("/api/wristband/payment", async (req, res) => {
  try {
    const { orderId, paymentMethod } = req.body;
    if (!orderId || !paymentMethod) {
      return res.json({ success: false, message: "Order ID and payment method required" });
    }

    const order = await WristbandOrder.findById(orderId);
    if (!order) return res.json({ success: false, message: "Order not found" });
    ...
    try {
      await Notification.create({
        userId: order.patientId,
        message: "Payment received! Your HealthMonitor Pro wristband order is confirmed.",
        type: "wristband_payment",
      });
    } catch (e) { console.log("Notification error:", e.message); }
    ...
    console.error("Wristband payment error:", err);
    return res.json({ success: false, message: "Server error" });
  }
});

// Admin: Get all wristband orders
app.get("/admin/wristband/orders", async (req, res) => {
  try {
    const orders = await WristbandOrder.find().sort({ createdAt: -1 });
    return res.json({ success: true, orders });
  } catch (err) {
    return res.json({ success: false, message: err.message });
  }
...
// Admin: Update wristband order status
app.put("/admin/wristband/order/:orderId/status", async (req, res) => {
  try {
    const { status } = req.body;
    ...
    if (status === "shipped") {
      ...
      order.trackingInfo.trackingNumber = "TRK" + Date.now().toString().slice(-8);
    } else if (status === "out-for-delivery") {
      order.trackingInfo.outForDeliveryAt = new Date();
    ...
    const statusMessages = {
      confirmed: "Your wristband order has been confirmed!",
      shipped: `Your wristband has been shipped! Tracking: ${order.trackingInfo.trackingNumber}`,
      "out-for-delivery": "Your wristband is out for delivery!",
      delivered: "Your HealthMonitor Pro wristband has been delivered! Connect it from your dashboard.",
      cancelled: "Your wristband order has been cancelled.",
    };
    ...
    console.error("Admin wristband status update error:", err);
    return res.json({ success: false, message: "Server error" });
  }
});
...
// Patient: Save a health reading from wristband
app.post("/api/health-reading", async (req, res) => {
  try {
    const { patientId, heartRate, spo2, ax, ay, az } = req.body;
    if (!patientId) return res.json({ success: false, message: "patientId required" });

    // Upsert: update if a reading exists within last 3 seconds, else create new
    const recentCutoff = new Date(Date.now() - 3000);
    const existing = await HealthReading.findOne({
      patientId,
      timestamp: { $gte: recentCutoff },
    });

    if (existing) {
      existing.heartRate = heartRate || 0;
      existing.spo2 = spo2 || 0;
      existing.ax = ax || 0;
      existing.ay = ay || 0;
      existing.az = az || 0;
...
// Doctor: Get latest health reading for a patient
app.get("/api/health-reading/:patientId/latest", async (req, res) => {
  try {
    // Check if patient has a delivered wristband
    const hasDevice = await WristbandOrder.findOne({
      patientId: req.params.patientId,
      status: "delivered",
    });

    if (!hasDevice) {
      return res.json({ success: true, hasDevice: false, reading: null });
    }
    ...
    const isRecent = reading && (Date.now() - new Date(reading.timestamp).getTime()) < 30000;
